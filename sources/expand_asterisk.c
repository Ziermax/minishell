...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/20 16:09:40 by mvelazqu          #+#    #+#             */
/*   Updated: 2024/07/22 03:27:51 by mvelazqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		{
//			printf("Searching mid in this: %s, this: %s\nRET:%s\n",
//				file, rules[i], current);
//		}
...
char	**expand_asterisk(char *string)
{
	...
	expan_rules = ultra_split(string, no_skip, next_asterisk);
	if (!expan_rules)
		return (NULL);
	files = get_file_in_dir(".");
	if (!files)
		return (free_split(expan_rules), NULL);
	matched_files = find_matched_files(expan_rules, files);
	free_split(files);
	free_split(expan_rules);
...
int	main(int argc, char **argv)
{
	...
	if (argc != 2)
		return (0);
	expansion = expand_asterisk(argv[1]);
	if (!expansion)
